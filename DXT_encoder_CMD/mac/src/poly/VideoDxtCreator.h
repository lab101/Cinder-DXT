...
namespace poly {
  ...
  public:
    VideoDxtCreator(int version = VIDEO_DXT_VERSION_0_0_0);
    ~VideoDxtCreator();
  ...
  inline int VideoDxtCreator::writeU32(uint32_t v) {
    ...
    if (!ofs.is_open()) {
      SX_ERROR("Cannot writeU32 because the file isn't opened.");
      return -1;
    }
  ...
  inline int VideoDxtCreator::rewriteU32(uint32_t offset, uint32_t v) {
    ...
    if (!ofs.is_open()) {
      SX_ERROR("Cannot rewriteU32 because the output file hasn't been opened.");
      return -1;
    }
    ...
    if (!ofs) {
      SX_ERROR("Failed to set the write position.");
      return -2;
    }
    ...
    if (!ofs) {
      SX_ERROR("Failed to jump back to the rewrite start position.");
      return -7;
    }
